    ...
    fn format_directory_inner(directory: &Path, max_width: Option<usize>) -> String {
        ...
        if let Some(max_width) = max_width {
            ...
            if UnicodeWidthStr::width(formatted.as_str()) > max_width {
                return Self::center_truncate_path(&formatted, max_width);
            }
        }
...
mod tests {
    ...
    fn test_center_truncate_doesnt_truncate_short_path() {
        let path = "~/workspace";
        let truncated = SessionHeaderHistoryCell::center_truncate_path(path, 40);
    ...
    fn test_center_truncate_truncates_long_path() {
        let path = "~/hello/the/fox/is/very/fast";
        let truncated = SessionHeaderHistoryCell::center_truncate_path(path, 24);

        assert_eq!(truncated, "~/hello/the/…/very/fast");
    }

    #[test]
    fn test_center_truncate_handles_long_segment() {
        let path = "~/supercalifragilisticexpialidocious";
        let truncated = SessionHeaderHistoryCell::center_truncate_path(path, 18);

        assert_eq!(truncated, "~/…cexpialidocious");
    }
    ...
    fn session_header_directory_center_truncates() {
        ...
        assert_eq!(formatted, "~/hello/the/…/very/fast");
    }
    ...
    fn session_header_directory_front_truncates_long_segment() {
        ...
        assert_eq!(formatted, "~/…cexpialidocious");
    }
